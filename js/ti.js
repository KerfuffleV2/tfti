// Transcrypt'ed from Python, 2019-07-26 09:07:30
...
export var TI =  __class__ ('TI', [object], {
	...
	get renderoneoff () {return __get__ (this, function (self, uniqueitems) {
		...
		for (var [c1buildable, c2buildable, item] of sorted (oneoff, __kwargtrans__ ({reverse: true, key: (function __lambda__ (i) {
			...
			var c = item.combine;
			result.append (self.rendercomponentstr (c [0], c [1], __kwargtrans__ ({minitclass: (!(c1buildable) ? 'showunbuildable' : ''), minibclass: (!(c2buildable) ? 'showunbuildable' : ''), imgclass: (item.score < SCORE_THRESHOLD ? 'showunbuildable lowscore' : 'showunbuildable')})));
		}
